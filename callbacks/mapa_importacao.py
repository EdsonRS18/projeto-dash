...
def update_mapa_importacao(
    ...
    for col in ['NOME_NOTI', 'NOME_INFE', 'NOME_RESI']:
    ...
    if selected_direction == SankeyDirection.NOTIFICATION_TO_INFECTION.value:
        source_col, target_col = 'SIGLA_NOTI', 'SIGLA_INFE'
    ...
    df['_SRC_NAME'] = df[source_nome].str.upper()
    df['_TGT_NAME'] = df[target_nome].str.upper()

    # =======================================================
    # 3) Mantém apenas fluxos que SAEM do estado selecionado
    # =======================================================
    df = df[df[source_col] == estado_selecionado]
    df = df[df['_SRC_NAME'] != df['_TGT_NAME']]

    # =======================================================
    # 4) 🔥 FILTRO DO SLIDER (IGUAL AO SANKEY)
    # =======================================================
    ...
    fig.add_annotation(
    text=(
        "<i>Os casos de notificação de municípios fora do estado selecionado não são exibidos no mapa.<br> "
        "Exemplo: ao selecionar o estado “Pará”, municípios como Manaus aparecerão com 0 notificações.</i>"
    ),
    x=0.5,
